...
export default class action {
    ...
    treeAdd = async () => {
        ...
        const ret = await this.modal.show({
            title: '新增',
            children: createAppElement('sys-entity-group', {
                parent: entityGroup
            }),
            bodyStyle: {
                height: 200
    ...
    add = async () => {
        this.component.props.setPortalContent && this.component.props.setPortalContent('新增实体', 'sys-entity')
    }

    modify = row => async () => {
        this.component.props.setPortalContent && this.component.props.setPortalContent('修改实体', 'sys-entity', {entityId: row.id})
    }
